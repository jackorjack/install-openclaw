...
function Write-Title { Write-Host ""; Write-Host "============================================================" -ForegroundColor Cyan; Write-Host "  $args" -ForegroundColor Cyan; Write-Host "============================================================" -ForegroundColor Cyan; Write-Host "" }
...
function Test-CNNetwork {
    try {
        $response = Invoke-WebRequest -Uri "https://registry.npmmirror.com/" -TimeoutSec 3 -UseBasicParsing
        return $true
    } catch {
        return $false
    }
...
function Test-VersionGTE {
    ...
    # VersionA >= VersionB 返回 $true
    $a = [System.Version]::new($VersionA)
    $b = [System.Version]::new($VersionB)
    return $a -ge $b
}
...
function Install-NodeJS {
    ...
    if ($winget) {
        ...
        try {
            ...
            $currentMajor = Get-NodeMajorVersion
            if ($currentMajor -ge $NODE_MIN_MAJOR) {
                Write-Info "Node.js $(Get-NodeFullVersion) 安装成功"
    ...
    # 方式3: 手动下载安装
    Write-Info "手动下载 Node.js v$NODE_RECOMMENDED_MAJOR LTS 安装包..."

    $nodeUrl = "https://nodejs.org/dist/v$NODE_RECOMMENDED_MAJOR.x/node-v$NODE_RECOMMENDED_MAJOR.x-x64.msi"
    $nodeMirrorUrl = "$NODE_MIRROR/v$NODE_RECOMMENDED_MAJOR.x/node-v$NODE_RECOMMENDED_MAJOR.x-x64.msi"

    if (Test-CNNetwork) {
        Write-Info "使用国内镜像下载: $NODE_MIRROR"
        $downloadUrl = "https://npmmirror.com/mirrors/node/v${NODE_RECOMMENDED_MAJOR}.x/"
    } else {
        $downloadUrl = "https://nodejs.org/dist/v${NODE_RECOMMENDED_MAJOR}.x/"
    }

    Write-Info "请手动下载并安装 Node.js (LTS):"
    Write-Host "  地址: $downloadUrl" -ForegroundColor Cyan
    Write-Host ""
    Write-Info "下载安装后请重新运行本脚本"
...
function Install-OpenClaw {
    ...
    if (-not $newVersion) {
        Write-ErrorMsg "安装后无法获取 OpenClaw 版本，安装可能未成功"
...
function Show-PostInstallHelp {
    ...
    if ($Script:NodeInstalledByUs) {
        Write-Warn "Node.js 是刚安装的，请重新打开 PowerShell 窗口以使 PATH 生效"
    }
}
